...
`neocache` facilitates robust sampling of the Twitter graph. The basic idea is to save any data into a local cache as you as you get it. `neocache` is build on top of [`socialsampler`](https://github.com/alexpghayes/socialsampler) and [`rtweet`](https://rtweet.info/).
...
``` bash
docker run --name neo4j --env NEO4J_AUTH=neo4j/password --publish=7474:7474 -d neo4j:3.5.21
```
...
Please note that the `neocache` project is released with a [Contributor Code of Conduct](CODE_OF_CONDUCT.md).
